...
// const columns = [
//   { id: 'attendanceId', label: 'Attendance ID', minWidth: 170 },
...
// function createData(attendanceId, employeeId, check_in, check_out) {
//   return { attendanceId, employeeId, check_in, check_out };
// }
...
//       return String(row.attendanceId).includes(searchValue) || row.employeeId.toLowerCase().includes(searchValue.toLowerCase()) ;
//     });
...
import { redRiver } from 'fontawesome';
...
const headCells = [
  { id: 'employeeId', label: 'Attendance ID', minWidth: 100 },
...
export default function EnhancedTable() {
  ...
  //     {item.employee_attendance && item.employee_attendance.map((data) => {
  //       attendanceData.push([
  //         {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = rows.map((n) => n.attendanceId);
      setSelected(newSelected);
      return;
    }
    setSelected([]);
  };
  const handleClick = (event, attendanceId) => {
    const selectedIndex = selected.indexOf(attendanceId);
    let newSelected = [];

    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, attendanceId);
  ...
  const isSelected = (attendanceId) => selected.indexOf(attendanceId) !== -1;
  ...
  const requestSearch = (searchValue) => {
    const filteredRows = fetchedData.filter((row) => {
      return String(row.attendanceId).includes(searchValue) || String(row.employeeId).includes(searchValue);
    });
    setRows(filteredRows);
  };
  const cancelSearch = () => {
          ...
          >
            ...
            <TableBody>
              ...
              {stableSort(fetchedData, getComparator(order, orderBy))
                .slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                .map((row, index) => {
                  const isItemSelected = isSelected(row.attendanceId);
                  const labelId = `enhanced-table-checkbox-${index}`;

                  return (
                    <TableRow
                      hover
                      onClick={(event) => handleClick(event, row.attendanceId)}
                      role="checkbox"
                    ...
                    >
                      ...
                      <TableCell center="right">{row.employeeId}</TableCell>
