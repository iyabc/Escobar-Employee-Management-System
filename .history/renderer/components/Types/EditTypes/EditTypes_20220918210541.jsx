...
export default function EditTypes({ editSuccessAction, employeeTypeId, employeeTypeName }) {
    ...
    const handleChange = (e) => {
        setNewEmployeeType({...newCategoryData, [e.target.name]:capitalizeData(e.target.value)});
    }

    const handleSubmit = () => {
        rest.update(
            `${INITIAL_URL}/employee-type/update/${newEmployeeType.expenseCategoryId}`,
            newCategoryData,
            editSuccessAction,
            `Successfully edited ${newEmployeeType.expenseCategoryName}`
        )
    }
