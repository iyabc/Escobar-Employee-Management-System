...
export default function AddEmployeeModal() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      .get("http://localhost:8080/api/v1/employee-type/active");
  ...
  useEffect(() => {
    const getData = async () => {
      const data = await axios
      .get("http://localhost:8080/api/v1/employee-position/active");
              ...
              <FormLabel>Employee Type</FormLabel>
                <RadioGroup value={type} onChange={typeChange} >
                  {employeeTypes.map((item) => {
                      return (
                        <FormControlLabel value={item.employeeTypeName} control={<Radio />} label={item.employeeTypeName} />
                      )
                  })}
                </RadioGroup>
          </FormControl>
              ...
              <FormLabel>Employee Position</FormLabel>
                <RadioGroup value={position} onChange={positionChange}>
                  {employeePositions.map((item) => {
                    return (
                      <FormControlLabel value={item.employeePositionName} control={<Radio />} label={item.employeePositionName} />
                    )
                  })}
                </RadioGroup>
          </FormControl>
