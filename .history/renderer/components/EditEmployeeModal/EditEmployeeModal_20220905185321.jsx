...
function EditEmployeeModal({id, last_name, first_name, employee_contact, employee_address, employee_type, employee_position, superior_employee, date_employed}) {
  ...
  function checkIfSuperior(emp_type) {
    if(emp_type != "Owner"){
      return (
        <TextField name="superior_employee" label="Superior Name" variant="standard" defaultValue={superior_employee} onChange={onChangeInput} error={errorsuperior_employee} fullWidth required />
      )
    }
  }
  //handle input
  ...
  const [erroremployee_address, seterroremployee_address] = useState("");
  const [erroremployee_contact, seteerroremployee_contact] = useState("");
  const [errorsuperior_employee, seterrorsuperior_employee] = useState("");
  const textFields = [
    {
      id:1,
      name:"last_name",
  ...
  const onChangeInput = (e) => {
    if(e.target.name == "last_name") {  
      if(onlyLetters(e.target.value)){
        seterrorlast_name("Letters only.");
      }else {
    ...
    if(e.target.name == "employee_address"){
      if(onlyLetterNumbers(e.target.value)){
