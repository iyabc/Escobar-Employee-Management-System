...
function EditEmployeeModal({id, last_name, first_name, employee_contact, employee_address, employee_type, employee_position, superior_employee, date_employed}) {
  ...
  const validate = () => {
    let temp = {};
    temp.last_name = onlyLetters(values.last_name) ? "" : errorMessages[values.last_name];
    temp.first_name = onlyLetters(values.last_name) ? "" : errorMessages[values.last_name];
    temp.employee_address = onlyLettersNumbers(values.employee_address) ? "" : errorMessages[values.employee_address];
    temp.employee_contact = onlyNumbers(values.employee_contact) && values.employee_contact.length == 11  ? "" : errorMessages[values.employee_address];
    temp.superior_employee = onlyLetters(values.superior_employee) ? "" : errorMessages[values.superior_employee];
    setErrors({ ...temp });
    return Object.values(temp).evert(x => x == ""); 
  }
  //handle submit
